	...
	registerAccount() {
		if (this.reg.passWrd1 != this.reg.passWrd2) {
			...
			this.afAuth.auth.createUserWithEmailAndPassword(this.reg.email, this.reg.passWrd1)
				.then(res => this.regSuccess(res)).catch(err => this.displayAlert('Error!', err));
		}
	}

	regSuccess(result) {
		this.userService.logOn(this.reg.email, this.reg.passWrd1).then(res => this.navCtrl.push(HomePage));
	}
}
